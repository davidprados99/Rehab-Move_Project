...
def get_physio_by_email(db: Session, email: str):
    return db.query(models.Physio).filter(models.Physio.email == email).first()
...
def create_physio(db: Session, physio: schemas.PhysioCreate):
    ...
    db_physio = get_physio_by_email(db, email=physio.email)
    if db_physio:
...
def update_physio(db: Session, id: int, physio_update: schemas.PhysioUpdate):
    ...
    if physio_update.email is not None and physio_update.email != db_physio.email:
        existing_physio = get_physio_by_email(db, email=physio_update.email)
        if existing_physio:
            raise ValueError("A physio with this email already exists.")
...
def create_patient(db: Session, patient: schemas.PatientCreate):
    ...
    db_patient = get_patient_by_email(db, email=patient.email)
    if db_patient:
...
def update_patient(db: Session, id_patient: int, patient_update: schemas.PatientUpdate):
    ...
    if patient_update.email is not None and patient_update.email != db_patient.email:
        existing_patient = get_patient_by_email(db, email=patient_update.email)
        if existing_patient:
            raise ValueError("A patient with this email already exists.")
...
def get_appointments_with_patients_and_physio(db: Session, physio_id: int):
    return db.query(models.Appointment)\
            .options(joinedload(models.Appointment.patient))\
            .options(joinedload(models.Appointment.physio))\
